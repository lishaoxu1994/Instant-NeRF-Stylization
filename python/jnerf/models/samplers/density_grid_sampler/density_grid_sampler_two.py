...
class DensityGridSampler_two(nn.Module):
    def __init__(self, cfg_dataset_obj, content_flag, update_den_freq=16, update_block_size=5000000):
        super(DensityGridSampler_two, self).__init__()
        self.cfg = get_cfg()
        self.content_flag = content_flag
        self.model = self.cfg.model_obj
        #self.dataset = self.cfg.dataset_obj
        self.dataset = cfg_dataset_obj
    ...
    def sample(self, img_ids, rays_o, rays_d, rgb_target=None, is_training=False):
        ...
        if self.using_fp16:
            with jt.flag_scope(auto_mixed_precision_level=5):
                nerf_outputs = self.model(coords_pos, coords_dir, self.content_flag).detach()
    ...
    def update_density_grid_nerf(self, decay: float, n_uniform_density_grid_samples: int, n_nonuniform_density_grid_samples: int):
        ...
        with jt.no_grad():
            ...
            for i in range(0,self._density_grid_positions.shape[0],bs):
                if self.using_fp16:
                    with jt.flag_scope(auto_mixed_precision_level=5):
                        res.append(self.model.density(self._density_grid_positions[i:i+bs], self.content_flag))
                else:
                    res.append(self.model.density(self._density_grid_positions[i:i+bs], self.content_flag))
