...
class Runner_two_img():
    def __init__(self):
        self.cfg = get_cfg()


        self.style_path = os.path.join(self.cfg.image_folder, self.cfg.image_name)
        self.styleimage = cv2.imread(self.style_path)
        self.styleimage = cv2.cvtColor(self.styleimage,cv2.COLOR_RGB2BGR)
        #self.style_volume = self.image2volume(self.styleimage)
        ...
        print(self.cfg.exp_name+'_and_' + self.cfg.image_name)
        ...
        self.save_path          = os.path.join(self.cfg.log_dir, self.exp_name + '_and_' + self.cfg.image_name)
    ...
    def image2volume(self, image):
        size = image.shape
        pixel_size = 0.5/size

        image = cv2.resize(image,[128,128])
        volume = jt.zeros([256,256,256,3])
        alpha = -10*jt.ones([256,256,256,1])
        volume[126:131,64:192,64:192,:] = image
        alpha[126:131,64:192,64:192,:] = 10
        volume_a = jt.concat([volume,alpha],3)
        return volume_a


    def style2outputs(self, pos, dir, image):
        size = image.shape
        if size[0] > size[1]:
            add1 = int((size[0]-size[1])/2)
            add2 = size[0]-size[1]-add1
            image = cv2.copyMakeBorder(image,0,0,add1,add2,cv2.BORDER_CONSTANT,value=(0,0,0))
        if size[0] < size[1]:
            add1 = int((size[1]-size[0])/2)
            add2 = size[1]-size[0]-add1
            image = cv2.copyMakeBorder(image,add1,add2,0,0,cv2.BORDER_CONSTANT,value=(0,0,0))
        image = jt.array(image)/255.0

        size = image.shape[0]
        image[0,:,:] = 0
        image[:,0,:] = 0
        image[int(size*1-1),:,:] = 0
        image[:,int(size*1-1),:] = 0
        
        pos[:,2] = (1- pos[:,2]-0.25)*2*size
        pos[:,1] = (1- pos[:,1]-0.25)*2*size
        pos[:,0] = (1- pos[:,0]-0.25)*2*size

        
        pos[:,0] = (pos[:,0].clamp(0, int(size*1-1)))
        pos[:,1] = (pos[:,1].clamp(0, int(size*1-1)))
        pos[:,2] = (pos[:,2].clamp(0, int(size*1-1)))
        pos = pos.int()
        #pos_flag = jt.ones([pos.shape[0],1])

        outputs_target = image[pos[:,1],pos[:,2],:]
        outputs_target[pos[:,0]<240,:] = 0
        outputs_target[pos[:,0]>270,:] = 0

        alpha = -10*jt.ones([outputs_target.shape[0],1])
        test = jt.sum(outputs_target,1)
        alpha[test>0] = 10
        outputs_target = jt.concat([outputs_target,alpha],1).float16()
    ...
    def train(self):
        for i in tqdm(range(self.start, self.tot_train_steps)):
            ...
            if i%2 == 0:
                ...
                network_outputs_target = self.style2outputs(pos, dir,self.styleimage).stop_grad()
                rgb_fun = jt.nn.Sigmoid()

                network_outputs[:,:3] = rgb_fun(network_outputs[:,:3])
                loss = self.loss_func(network_outputs, network_outputs_target)/16
                #loss = self.loss_func(rgb, rgb_target)
    ...
    def test_style(self, style_path, content_flag=1):
        ...
        for img_i in tqdm(range(0,dataset_temp["test"].n_images,1)):
            with jt.no_grad():
                ...
                if content_flag:
                    self.save_img(style_path+f"/{self.exp_name}_2_{self.cfg.image_name}_r_{img_i}.png", img, alpha)
                else:
                    self.save_img(style_path+f"/{self.cfg.image_name}_2_{self.exp_name}_rs_{img_i}.png", img, alpha)                    
            ...
            with jt.no_grad():
                ...
                if save_img:
                    if content_flag:
                        ...
                        self.save_img(save_path+f"/{self.cfg.image_name}_rs_{img_i}.png", img, alpha)
                        #if dataset_temp["test"].have_img:
